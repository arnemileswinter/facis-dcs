...
// settlementColumns is shared by the reads so a row always lands in the same
// struct fields regardless of the table's physical column order.
const settlementColumns = `did, party_did, is_local, document_hash, contract_version, settled_at, jades_signature, shipped_at, received_at`

func (r PostgresSyncRepository) UpsertSettlementStatement(ctx context.Context, tx *sqlx.Tx, stmt db.SettlementStatement) error {
	statement := `
        INSERT INTO contract_settlement_statements
            (did, party_did, is_local, document_hash, contract_version, settled_at, jades_signature, shipped_at, received_at)
        VALUES ($1, $2, $3, $4, $5, $6, $7, NULL, CURRENT_TIMESTAMP)
        ON CONFLICT (did, party_did) DO UPDATE SET
	...
	_, err := tx.ExecContext(ctx, statement,
		stmt.DID, stmt.PartyDID, stmt.IsLocal, stmt.DocumentHash, stmt.ContractVersion, stmt.SettledAt, stmt.JadesSignature)
	return err
}

func (r PostgresSyncRepository) GetSettlementStatements(ctx context.Context, tx *sqlx.Tx, did string) ([]db.SettlementStatement, error) {
	query := `
        SELECT ` + settlementColumns + `
        FROM contract_settlement_statements
        WHERE did = $1
    `
	var statements []db.SettlementStatement
	if err := tx.SelectContext(ctx, &statements, query, did); err != nil {
		return nil, err
	}
	return statements, nil
}

func (r PostgresSyncRepository) GetUnshippedSettlements(ctx context.Context, tx *sqlx.Tx) ([]db.SettlementStatement, error) {
	query := `
        SELECT ` + settlementColumns + `
        FROM contract_settlement_statements
    ...
    `
	var statements []db.SettlementStatement
	if err := tx.SelectContext(ctx, &statements, query); err != nil {
		return nil, err
	}
	return statements, nil
}

func (r PostgresSyncRepository) MarkSettlementShipped(ctx context.Context, tx *sqlx.Tx, did, partyDID string) error {
	statement := `
        UPDATE contract_settlement_statements
        SET shipped_at = CURRENT_TIMESTAMP
        WHERE did = $1 AND party_did = $2
    `
	_, err := tx.ExecContext(ctx, statement, did, partyDID)
	return err
}
