...
| VAL-18 Workflow Semantic Hub/PDP rule enforcement | DCS-FR-PACM-03; ADR-5/-8/-9/-23; stakeholder input 2026-07-27 | Contract at each defined gate plus its active or artifact-pinned Semantic Hub shapes/profile, embedded ODRL rules and the configured external audit-executor decision | Error-severity Hub violations block the defined submission/signature gates; DCS technical checks remain evidence; the configured executor produces separately attributable final PAC findings and any unavailable or invalid executor result fails closed | Rule activation/rollback, pinned old version, missing/unavailable Hub, conflicting context, warning/error boundary, unavailable/malformed/mismatched executor result, empty valid result, replacement endpoint | **partial** — the external executor boundary is verified by `features/08_audit_compliance/external_pac_audit_executor.feature:8-126` (9/9 scenarios, 111/111 steps on 2026-07-28), including shipped ORCE, configuration-only replacement, exactly-one dispatch and fail-closed behavior; this does not by itself close the remaining workflow-gate and Semantic Hub coverage listed in this row |
