...
- **Submit ist bewusst überladen:** Seine Wirkung hängt vom aktuellen
  ...
  Task-Orchestrierung.
- **Withdraw** ist nur bis zur Freigabe möglich — ein einmal
  freigegebener Vertrag kann nicht mehr zurückgezogen, nur noch
  terminiert werden.
...
- **Renew** ist kein Zustandsübergang: Die Verlängerung erzeugt eine
  ...
  Original-DID und -Version; das Original bleibt unverändert, und die
  Signatare der Vorperiode werden aus dem neuen Entwurf entfernt — ein
  ungezeichneter Entwurf darf niemanden als Unterzeichner ausweisen.
...
1. **Lokale RBAC-Rollen** bestimmen, welcher lokal authentifizierte
   ...
   jeweils mit „Sys."-Pendants für maschinelle Aufrufer (Kapitel 05).
   Audit-Lesezugriff haben Auditor, Archive Manager und Compliance
   Officer; Administrationsrechte der Sys. Administrator.
...
instanzübergreifenden Vertrags autorisiert sich nicht über lokale Tasks,
sondern über ihre Eigenschaft als benannte Gegenpartei (siehe 4.5).
...
Die Verhandlung findet im Zustand `NEGOTIATION` (bzw. auf einem
eingegangenen Angebot in `OFFERED`) statt und dreht sich um
Change-Requests:
...
allen autorisierten Verhandlern derselben Partei geteilt — jeder setzt
die gemeinsame Verhandlungsposition der Partei fort. Ein Draft ändert
keinen Vertragszustand, erhöht keine Version, erzeugt kein
...
did:web-Challenge-Response-Signatur, nicht per Session-Token — es gibt
...
ohne designiertes Ziel deployt schlicht nicht — das ist ein normaler
Ausgang, kein Fehler: Der automatische Trigger nach Signaturabschluss tut
dann nichts; ein manuell angeforderter Deploy wird mit sprechender
Begründung abgewiesen, ebenso bei einem unbekannten oder deaktivierten
Ziel.
...
Payload, den das Zielsystem selbst nachrechnen kann; ein
fehlgeschlagener Versand wird am Datensatz vermerkt und vom
Compliance-Monitor als Risiko gemeldet.
...
Nur das Ziel, an das ein Deployment ging, kann es quittieren — ein
...
- **Ablauf.** Der Hintergrundjob persistiert den `EXPIRED`-Zustand und
  ...
  Folgehandlung ist derzeit ein manueller bzw. über Webhooks
  orchestrierter Schritt.
