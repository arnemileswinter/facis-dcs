...
verifizierbaren DID-Dokumenten — nie aus Netzwerkposition oder
Konfigurationsnähe (Zero-Trust-Modell, vgl. Kapitel 01).
...
der verifizierten Wallet-Vorlage; Hydra dient als Token-Fabrik, deren Login-
und Consent-Challenges das Backend über die Hydra-Admin-API akzeptiert und
dabei die verifizierten Claims (Subjekt, Organisation, Rollen) in die
Token-Session übernimmt.
...
4. **Statusliste.** Der Widerrufsstatus wird gegen die referenzierte
   ...
   Ablehnung, nicht zur Annahme mit Warnung.
...
- **IP-Lockout:** Fehlgeschlagene Token-Validierungen werden je Quell-IP
  ...
  dagegen eine Autorisierungs-, keine Authentifizierungsentscheidung und
  zählt nicht zum Lockout — sie wird als authentifiziertes
  Zugriffsereignis auditiert.
- **Audit-Trail:** Zugriffs- und Präsentationsversuche (erfolgreich wie
  fehlgeschlagen) werden persistiert und in den
...
Rotieren und Widerrufen sind auditierte Betriebs-, keine
Deployment-Handlungen.
...
Fehlkonfiguration unterschieden — der Prozess wartet auf das Token,
statt sofort abzustürzen, weil bei einer frischen Installation die
Provisionierung noch laufen kann (Kapitel 10).
...
Schlüssel nicht zusammen oder fehlt dem Token die Ableitungsfähigkeit —
in beiden Fällen wäre kein gespeichertes Artefakt lesbar, und die Instanz
startet nicht.
...
Ein Eintrag ohne Zweckangabe wird beim Laden abgewiesen: Ihn stillschweigend
als „alle Zwecke" zu lesen wäre genau die Vermischung, die diese Trennung
beseitigt. Welche Aussteller welchen Zweck erhalten, ist
Betreiberentscheidung — in Produktion sind mehrere Login-Aussteller
normal.
...
dort fehlt, wird unabhängig vom Zweck abgelehnt — ein vertrauenswürdiger
Aussteller kann damit nicht für eine Partei sprechen, die ihm niemand
zugestanden hat. Ist ein Aussteller selbst die Mandantenverwaltung seines
Deployments, deklariert er einen expliziten Platzhalter; eine fehlende
Liste bedeutet niemals „beliebig". PID-Aussteller sind ausgenommen: Ein
Identitätsnachweis attestiert eine Person, keine Organisation.
...
Laden** abgewiesen — ein Deployment erfährt von einer nicht
unterstützten Trust-Konfiguration beim Start, nicht wenn die erste
Wallet ankommt. Neue Registrierungsverfahren erreichen den Verifier über
`orce` ohne Codeänderung.
