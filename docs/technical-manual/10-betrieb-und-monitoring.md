...
- **Der Port ist von Anfang an belegt.** Ein Bootstrap-Server beantwortet
  ...
  startenden Prozess von einem betriebsbereiten DCS und sieht keinen
  Verbindungsabbruch.
...
- **Der Selbsttest der Schlüsselvereinbarung ist entscheidungsrelevant:**
  Er beweist, dass der im DID-Dokument publizierte Schlüssel und der
  Token-Schlüssel zusammenpassen und dass das Token die nötige Ableitung
  beherrscht. Schlägt er fehl, wäre kein gespeichertes Artefakt lesbar,
  und die Instanz startet nicht (Kapitel 07).
...
Für den Betrieb wichtig: **Der Backend-Container wird ausschließlich auf
Bereitschaft geprüft. Ein Liveness-Signal wird nicht abgefragt.** Ein
Prozess, der zwar läuft, aber nicht mehr arbeitet, wird deshalb nie
automatisch neu gestartet. Antwortet er weiterhin auf dem
Bereitschaftsendpunkt, bleibt er sogar im Service-Endpoint und nimmt
...
eine Lebendigkeitsprobe will, fügt sie bewusst hinzu und muss dabei den
Startfall abdecken.
...
wenn eine Nachbarkomponente gestört ist, also genau dann, wenn er
gebraucht wird (Kapitel 09).
...
Anfragebudgets. Ein Scraping-Ziel bringt die Instanz nicht mit; das
Einsammeln der Metriken richtet der Betreiber in seiner eigenen
Monitoring-Infrastruktur ein.
...
Backend **nicht** als Prometheus-Zeitreihen. Die entsprechenden
