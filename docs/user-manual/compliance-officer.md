...
1. **Run monitoring sweep** **(1)** startet den Überwachungslauf über den
   gesamten Bestand. Während der Lauf läuft, sagt die Ansicht das ausdrücklich;
   der Zeitpunkt des letzten Laufs steht anschließend darunter („Last checked:
   …").
...
- **Noch nicht gelaufen** — ein Hinweis fordert zum Start des Laufs auf.
- **Lauf ohne Befund** — „completed … no compliance risks".
- **Filter ohne Treffer** — ein eigener Hinweis, dass keine Zeile dem
  aktuellen Filter entspricht.
- **Technischer Fehler** — eine rote Fehlermeldung mit dem Grund; es wird
  keine leere Ergebnisliste vorgetäuscht.
...
Unterhalb der Überwachung melden Sie festgestellte Verstöße als formalen
Vorfallbericht.
...
Nach erfolgreicher Übermittlung bestätigt die Ansicht die Meldung **(1)**; der
Bericht ist damit dauerhaft dem Vertrag zugeordnet und für Prüfungen
nachvollziehbar.
...
- Solange die Prüfung offen ist, darf der Vertrag weder inhaltlich noch im
  ...
  abgelehnt — die frühere Genehmigung gilt nicht für den geänderten Vertrag.
  Der Übergang muss dann für den aktuellen Stand neu angefordert werden.
- Eine zweite Genehmigung startet keine parallele Fortsetzung.

## Signaturprüfungen

Zusätzlich haben Sie Zugriff auf den **Compliance Viewer** zur Prüfung von
Signaturen signierter Verträge — siehe
