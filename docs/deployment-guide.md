...
| pdf-core | `pdf-core/Dockerfile` | C2PA-fähiger PDF-Compiler und -Verifier |
...
| Voraussetzung | Warum |
| --- | --- |
| Kubernetes-Cluster mit Ingress-Controller | Die `/.well-known`-Dokumente und die UI müssen extern erreichbar sein. Die mitgelieferten Beispielwerte für Issuer und `sharedServices` verwenden `nginx.ingress.kubernetes.io/*`-Annotationen und setzen damit **ingress-nginx** voraus; unter einem anderen Controller sind die Rewrite-Regeln zu ersetzen. |
...
| RBAC: `roles`/`rolebindings` im Namespace anlegen dürfen | Der HSM-Provisionierungs-Hook und die ACME-Erneuerung bringen eigene ServiceAccount/Role/RoleBinding mit. Ist das nicht erlaubt, gibt es einen Betriebsmodus ohne API-Zugriff — siehe A6.5. |
| Ressourcenrahmen | Eine vollständige Instanz sind rund zehn Workloads und zehn Services. Unter einer `ResourceQuota` ist besonders `limits.cpu` zu prüfen; unter einer `LimitRange` ist der Minimalwert pro Container zu prüfen. Beides erzeugt Fehlerbilder, die nicht wie Fehler aussehen — siehe A13. |
...
`helm` und `kubectl` müssen im `PATH` liegen — `deploy.sh` bricht sonst ab.
...
Werte-Set — sie gehört damit in einen Secret-Manager-Aufruf über `--set`, nicht
in eine Werte-Datei.
...
| --- | --- | --- |
| `go: module lookup disabled by GOPROXY=off` im Build | Die Goa-Codegenerierung im Build läuft hermetisch aus dem Modul-Cache. Eine Abhängigkeit fehlt in `go.sum`. | `go mod download all` im `backend/`-Verzeichnis nachziehen und `go.sum` committen. |
...
| --- | --- | --- | --- |
| `postgresql` | — | `postgresql.enabled` | Mitgelieferter Datenbankserver |
| `hydra` | — | `hydra.enabled` | OAuth2/OIDC für Browser- und Maschinen-Login |
| `nats` | — | `nats.enabled` | Event-Bus |
| `orce` | — | `orce.enabled` | Node-RED-Flows: Trust-PDP, Archiv-Notar, TSA, Contract-Target, Audit-Executor |
| `dss` | — | `dss.enabled` | EU-DSS-Signaturvalidierung |
...
| `kube-prometheus-stack` | — | `monitoring.enabled` | Prometheus/Grafana/Alertmanager |
| `traefik` | — | `traefik.enabled` | Ingress-Controller, nur für Testcluster |
...
`federatedCatalogue.enabled` — das ist der Konfigurationsblock des
DCS-Backends für ausgehende Aufrufe und unabhängig davon, wo der Katalog läuft.
...
| --- | --- | --- | --- |
| `values.yaml` | Basis, immer aktiv | — | Nichts direkt; sie ist bewusst nicht deploybar (keine Registry, kein Host, keine Identität) |
...
| `values.issuer.yml` | Credential-Issuer neben Instanz A | Eigenes Helm-Release des `orce`-Subcharts | `ingress.hosts[].host`, `ingress.className`, `ingress.tls` |
| `values.issuer2.yml` | Credential-Issuer neben Instanz B | wie oben | wie oben |
| `values.pid-issuer.yml` | PID-Issuer, einmal je Föderation | Eigenes Helm-Release des `orce`-Subcharts | `ingress.hosts[].host`, `ingress.className`, `ingress.tls` |

### A4.1 `values.prod.yml` und `values.acceptance.yml` sind Skelette

Beide Dateien beschreiben das **Muster** einer Umgebung, nicht ein
einsatzfähiges Deployment. Sie tragen Platzhalter (`dcs.example.org`,
...
Das ist keine Schwäche der Dateien, sondern ihr Zweck: Hostnamen, Registry,
Identität und Secret-Anbindung sind Deployment-Zustand und gehören in eine
Werte-Datei **außerhalb dieses Repositories**. Behandle die beiden Dateien als
Checkliste, nicht als Konfiguration.
...
lesender Ort belegt ist. Werte ohne Leser stehen in A5.8.
...
| `ipfs.tenantBaseURLSecretRef` | `IPFS_TENANT_BASE_URL` aus einem Secret | Wird gelesen |
...
| `ipfsClient.mfsBaseURL` | `IPFS_MFS_BASE_URL` | Ohne Angabe abgeleitet als `http://<release>-ipfs:5001`. **Dies ist der einzige wirksame Knopf für die MFS-Adresse** — siehe A5.8. |

Beide IPFS-Variablen sind beim Start zwingend; fehlen sie, bricht das Backend
mit `IPFS configuration missing` ab.
...
- **Systemrollen können den Katalog nicht durchsuchen.** Die
  Katalog-Endpunkte fordern menschliche Rollen. Ein Maschinen-Token erhält
  dort `insufficient permissions`. Diese Grenze ist beabsichtigt; sie wird
  nicht dadurch behoben, dass man einem Maschinen-Client menschliche Rollen
  gibt.
...
`systemClients` ist eine **Saat**, keine Laufzeitquelle: die Einträge werden
beim Start in die Maschinen-Identitäts-Registry übernommen, damit eine
Instanz ihre Aufrufer hat, bevor sich jemand anmeldet. Danach werden
...
| `signing.tsaURL` | `TSA_URL` | Beim Start zwingend. Leer bricht ab. Siehe A5.9 dazu, worauf diese URL per Vorgabe zeigt. |
| `signing.tsaTrustCertSecret` | `TSA_TRUST_CERT_FILE` = `/etc/dcs/tsa-trust/tsa-cert.pem`, plus Secret-Mount | Ersetzt das im Binary eingebettete TSA-Zertifikat. Das Secret muss den Schlüssel `tsa-cert.pem` tragen. Damit ist ein Wechsel der Zeitstempel-Autorität **eine Konfigurationsänderung, kein Neubau des Backends**. |
...
entstanden ist, eine spätere Werte-Änderung bleibt also wirkungslos.

### A5.5 Föderation und Vertrauen

| Wert | Gerenderte Wirkung | Wirkung im System |
| --- | --- | --- |
| `federation.trustPdpURL` | `DCS_TRUST_PDP_URL` (nur wenn gesetzt) | Der Policy-Endpunkt, gegen den jede eingehende und ausgehende föderierte Interaktion geprüft wird. **Fail-closed:** solange er leer ist, wird nichts versendet und nichts angenommen. Jede Instanz konsultiert nur ihren eigenen Endpunkt. |
| `oid4vp.trust.enabled` | Rendert `OID4VP_TRUST_DATA_PATH` | Ohne sie kennt die Instanz kein Vertrauensdokument und weist jede Credential-Vorlage ab |
| `oid4vp.trust.existingConfigMap`/`.existingConfigMapKey`/`.existingConfigMapMountPath` | Mount und Pfad des Vertrauensdokuments | Der bevorzugte Weg. Die ConfigMap wird nicht von Helm verwaltet und überlebt eine Deinstallation. |
| `oid4vp.trust.dataPath` | Pfad, wenn keine ConfigMap gesetzt ist | Zeigt auf das im Image liegende Dev-Fixture; siehe A7.4 |
| `oid4vp.trust.x5cAnchorsPath` | `OID4VP_X5C_TRUST_ANCHORS_PATH` | Vertrauensanker, gegen die eine x5c-tragende Credential-Kette geprüft wird. Leer heißt: eine x5c-tragende Credential wird abgewiesen, nie ihrem eigenen Blattzertifikat geglaubt. |
| `oid4vp.trust.xfscAllowUnsignedFallback` | `OID4VP_XFSC_ALLOW_UNSIGNED_FALLBACK=true` | Nur Dev/CI |
| `federatedCatalogue.enabled` | Rendert den gesamten FC-Konfigurationsblock | Alles-oder-nichts: das Backend fordert, dass API-URL, Client-ID, Client-Secret und Keycloak-Realm-URL gemeinsam gesetzt sind |
| `federatedCatalogue.apiURL` bzw. `.secretRef` | `FEDERATED_CATALOGUE_API_URL` | Muss den Servlet-Kontextpfad des fc-service enthalten — siehe A13 |
| `federatedCatalogue.oauth.clientID` | `FEDERATED_CATALOGUE_CLIENT_ID` | Muss ein Client sein, den der verwendete Keycloak-Realm tatsächlich anlegt. Der mitgelieferte Realm legt `federated-catalogue` an; die Chart-Vorgabe `dcs-fc-client` passt dazu **nicht** und führt gegen den mitgelieferten Katalog zu 401 |
| `federatedCatalogue.oauth.clientSecretSecretRef` bzw. `.clientSecret` | `FEDERATED_CATALOGUE_CLIENT_SECRET` | |
| `fcKeycloak.realmURL` | `FC_KEYCLOAK_REALM_URL` | Die **öffentliche** Realm-URL. fc-service prüft den `iss`-Claim des Tokens dagegen; ein über eine In-Cluster-Adresse geprägtes Token wird abgelehnt. Der mitgelieferte Realm heißt `federated-catalogue-realm` und liegt unter der Basis `/auth`, die URL hat also die Form `https://<host>/auth/realms/federated-catalogue-realm`. |
| `federatedCatalogue.remote.acknowledgeAdminAllTrustBoundary` | Freigabe des Renders | Zwingend `true`, wenn `federatedCatalogue.enabled` ohne `fcservice.enabled` gesetzt ist. Der Dienstzugang zum Katalog trägt derzeit `ADMIN_ALL` und ist keine Mandantentrennung; ein geteilter Katalog ist nur innerhalb einer gemeinsam vertrauenden Verwaltungsgrenze zulässig. |
| `fcservice.enabled` | Katalog, Keycloak, Fuseki und fc-postgres im eigenen Release | |
| `fcservice.realmProvision.enabled` | Realm-Import-Hook | Bei frischer Katalog-Datenbank erforderlich. Der Import läuft über die Admin-API, nicht über `--import-realm`. |
| `fcservice.keycloak.hostname` | Issuer-URL, die fc-service erwartet | Leer heißt: fc-service bildet sie aus dem In-Cluster-Service. Für echten Ingress ist das falsch. |
| `fcKeycloakApp.hostname` | Öffentlicher Hostname des mitgelieferten Keycloak | Muss mit `fcservice.keycloak.hostname` von Hand synchron gehalten werden |
| `fcservice.portal.enabled` | Demo-Portal des Katalogs | |
| `fcservice.route.path` | Zusätzliche Ingress-Regel auf `fc-service:8081` | Erlaubt einer Instanz in einem anderen Cluster, diesen Katalog zu erreichen |
...
| — (nur über `extraEnv`) | `STATUSLIST_LIST_ID` | Die Liste, in der neue Widerrufseinträge vergeben werden; Vorgabe ist Liste 1. Sie wird erst gesetzt, wenn Liste 1 voll ist — und erst, nachdem der Betreiber die Nachfolgeliste im Status-List-Dienst angelegt und in `status_list_cursors` registriert hat. Eine nicht registrierte Liste lässt die Vergabe hart fehlschlagen, statt einen Eintrag auszugeben, den niemand ausliefert. Ein nicht-positiver Wert bricht den Start ab. Es gibt dafür keinen Chart-Wert. |
...
| `pdfCore.replicaCount`, `.resources`, `.service.port` | pdf-core-Deployment | |
| `signing.tsaURL` | siehe A5.4 | |
...
| `global.workflowGateTokenSecretRef` bzw. `.workflowGateToken` | `PAC_WORKFLOW_GATE_EXECUTOR_BEARER_TOKEN` | |
...
| `autoscaling.enabled`, `.minReplicas`, `.maxReplicas`, `.targetCPUUtilizationPercentage` | HorizontalPodAutoscaler | |
...
| `podSecurityContext`, `securityContext`, `nodeSelector`, `tolerations`, `affinity`, `podAnnotations`, `podLabels`, `hostAliases` | Direkt in das Deployment übernommen | |
...
| `serviceMonitor.enabled` | ServiceMonitor auf `/metrics` | Mit Einschränkung — siehe A5.8 |
| `monitoring.enabled` und die Blöcke `grafana`, `prometheus`, `alertmanager`, `nodeExporter`, `kubeStateMetrics` | kube-prometheus-stack | Die Beispielwerte verwenden `storageClassName: local-path`; in einem Cluster ohne diese Klasse bleiben die PVCs ungebunden |
| `istio.enabled`, `.hosts`, `.gateways` | VirtualService statt Ingress | |
| `statusListLocalhostProxy.*` | Nginx-Sidecar im Backend-Pod | Ausschließlich für die BDD-Umgebung |

### A5.8 Deklarierte Werte ohne wirksamen Leser

Die folgenden Knöpfe existieren, wirken aber nicht wie erwartet. Sie sind
hier aufgeführt, damit niemand eine Konfiguration darauf stützt.

| Wert | Was tatsächlich passiert |
| --- | --- |
| `ipfs.mfsBaseURL`, `ipfs.mfsBaseURLSecretRef` | Werden von keinem Template gelesen. `IPFS_MFS_BASE_URL` kommt ausschließlich aus `ipfsClient.mfsBaseURL` bzw. dem In-Cluster-Fallback. In `values.prod.yml` und `values.acceptance.yml` ist `mfsBaseURLSecretRef` gesetzt — dort entsteht der Eindruck, die Adresse käme aus einem Secret; sie kommt aus dem Fallback. |
| `pdfCore.signing.signerKeyPEM` | Der Render verlangt den Wert, sobald `pkcs11.provisioning.enabled=false` und `pdfCore.signing.existingSecret` leer sind, und schreibt ihn als `signer-key-pem` in ein Secret. Kein Container liest diesen Schlüssel. pdf-core hält kein Schlüsselmaterial. Der Weg für Produktion ist `pdfCore.signing.existingSecret`. Der Geschwisterwert `x5ChainPEM` ist dagegen wirksam. |
| `pdfCore.contextIRI` | Wird als `PDF_CORE_CONTEXT_IRI` in das Backend-Deployment gerendert und von keinem Codepfad gelesen. Der wirksame Knopf für die IRI-Basis ist `pdfCore.ontologyBaseURL`. |
| `livenessProbe` | Das Backend-Template rendert einen Liveness-Block, sobald der Wert gesetzt ist — er ist aber in keiner Werte-Datei deklariert. **Der Backend-Container hat in keinem Deployment eine Liveness-Probe.** Das ist mit dem Bootstrap-Verhalten vereinbar (siehe A12.1); wer eine will, deklariert sie selbst. |
| `serviceMonitor.enabled` | Das gerenderte ServiceMonitor-Objekt trägt fest den Namespace `dcs-bdd` und den Selektor `release: prometheus`, unabhängig vom Release-Namespace. In jedem anderen Namespace ist es entweder unwirksam oder wird abgelehnt. Für ein Monitoring außerhalb der Testumgebung ist ein eigenes ServiceMonitor-Objekt anzulegen. |
| `pdfCore.signing.existingSecretSignerKeyKey` | In keinem Template referenziert |
| `serviceDiscovery.keycloakHost` | **Existiert nicht.** `serviceDiscovery` kennt genau zwei Felder: `postgresqlHost` und `natsHost`. Es gibt keinen Keycloak-Discovery-Knopf — die Anbindung an den Katalog-Keycloak läuft ausschließlich über `fcKeycloak.realmURL` und `fcservice.keycloak.hostname` |
| `autoscaling.targetMemoryUtilizationPercentage`, `hostNetwork`, `dnsPolicy`, `nameOverride`, `fullnameOverride`, `fcservice.keycloak.clientId`, `fcservice.keycloak.realm`, `ipfsDocumentManager.service.port` | Von Templates referenziert, aber in keiner Werte-Datei deklariert. Sie funktionieren, wenn man sie selbst setzt; sie sind nur nirgends dokumentiert vorgegeben. |

### A5.9 Die Zeitstempel-Autorität ist per Vorgabe lokal

`TSA_URL` ist ein Startgatter, und die naheliegende Annahme — die Instanz
spreche mit einer externen Zeitstempel-Autorität — trifft in der
Vorgabekonfiguration **nicht** zu.
...
- Ein Zeitstempel aus der Vorgabekonfiguration ist ein **Entwicklungs-
  ...
  Beweiswert braucht, richtet `signing.tsaURL` auf eine echte Autorität und
  hinterlegt deren CA-Zertifikat über `signing.tsaTrustCertSecret`.
- Beides ist Konfiguration. Ein Wechsel der Autorität erfordert **keinen
  Neubau des Backends**: das im Binary eingebettete Zertifikat ist nur der
  Vorgabewert, `TSA_TRUST_CERT_FILE` übersteuert ihn.
...
| `dcs-vc` | Signiert Lifecycle- und Federation-Credentials | |
| `dcs-oid4vp-jar` | Signiert OpenID4VP-Request-Objekte | |
...
der Wallet der unterzeichnenden Person; die Aufgabe des DCS ist, sie zu prüfen.
...
1. Das PKCS#11-Modul des Herstellers ist im Backend-Container erreichbar —
   über ein zusätzliches Volume aus `volumes`/`volumeMounts` oder ein
   abgeleitetes Image.
...
Kein Wert steht in diesem Dokument — nur Herkunft, Verbraucher und die Wirkung
seines Fehlens.
...
`systemToken` ist das In-Cluster-Dienstcredential, mit dem die
...
- Das Backend-Template fordert den Wert per `required` an. Ein Deployment ohne
  ihn rendert nicht. Ein im Repository mitgeliefertes Default gibt es damit
  nicht.
- Die Prüfung im Backend schränkt den Träger des Wertes nicht ein. Wer ihn
  ...
  Berechtigung. Dass er nur im Cluster zirkuliert, ist eine Eigenschaft des
  Deployments, nicht der Prüfung.
...
Ein Härtungsschritt, der über das Chart hinausgeht: Der Wert wird derzeit als
Klartext-`value` in das Deployment gerendert und ist damit für jeden lesbar,
der `get deployment` im Namespace darf. Eine `secretKeyRef` wäre die stärkere
Form; sie existiert in dieser Chart-Version nicht.
...
die eine unterzeichnende Person vorlegt — die Vollmacht (Power of Attorney) und
der Personenidentitätsnachweis (PID) — kommen aus OID4VCI-Issuern, die als
eigene Helm-Releases des `orce`-Subcharts laufen, jeweils mit eigenem Volume,
eigenem Schlüssel und eigener DID.

| Release | Werte-Datei | Stellt aus | Anzahl |
| --- | --- | --- | --- |
| `dcs-issuer` | `values.issuer.yml` | Vollmacht | Einer je DCS-Instanz, unter deren Hostnamen |
| `dcs-issuer` | `values.issuer2.yml` | Vollmacht | Für die zweite Instanz, in deren Namespace |
...
einen Satz — die Sätze beanspruchen überlappende Routen.
...
Unterzeichnenden akzeptiert, hat nichts bezeugt. Der Vollmachts-Issuer ist der
umgekehrte Fall — er spricht *für* eine Organisation darüber, wer für sie
zeichnen darf, und gehört deshalb zu ihr.

Der mitgelieferte PID-Issuer prüft niemanden. Die Credential, die er ausstellt,
ist keine EUDI-PID und darf nicht als solche präsentiert werden.

### A7.1 Was der Betreiber beisteuern muss

Das Repository liefert die Form eines Issuers, nie eine Identität:
...
| Pod bleibt in `ContainerCreating`, Event nennt eine fehlende ConfigMap | `existingConfigMap` zeigt auf einen nicht existierenden Namen | ConfigMap anlegen. Der Mount ist bewusst nicht `optional` — ein fehlendes Vertrauensdokument ist beim Start ohnehin fatal, und ein Mount-Fehler ist die besser diagnostizierbare Variante. |
...
| `peer` | Wird geprüft, wenn die Credential von einer Gegenpartei eintrifft — insbesondere in der Signaturzeremonie |
...
alle Deployments bereit sind — aber das Backend wird erst bereit, nachdem der
...
| Föderationsdokumente | `/.well-known/did.json`, `/.well-known/dcs-agreement-credential.json` und `/.well-known/dcs-federation-rules.md` liefern von **außen** 200 | Ein anderer Anspruch auf `/.well-known` — Hydras OIDC-Discovery — überdeckt sie. Im Cluster sieht alles korrekt aus; nur ein externer Abruf zeigt den 404. |
...
angepasst. `repoURL` und `targetRevision` sind auf das eigene Repository und
den auszurollenden Stand zu setzen.
...
`syncPolicy.automated` ist nicht gesetzt. Das ist eine Entscheidung, keine
Auslassung, und sie hat vier Gründe — drei aus dem Chart, einer aus dem
Betrieb:
...
2. **Der FC-Realm-Hook muss laufen, bevor die Anwendung gesund werden kann.**
   ...
   typischerweise erst aus, wenn der Sync **gesund** ist — aber das Backend
   behandelt einen fehlgeschlagenen Katalog-Sync als fatal und läuft in eine
...
**Der empfohlene Ablauf ist deshalb:** ArgoCD hält den Sollzustand, meldet
Drift und erzeugt den Sync-Auftrag; der eigentliche Sync wird ausgelöst
(manuell oder über eine Freigabe in der Pipeline). Für Erstinstallation und
für Upgrades, die die Provisionierungs-Hooks berühren, ist `deploy.sh` der
verlässlichere Weg, weil es die Reihenfolge kennt.
...
in A8.3 — insbesondere die externen `/.well-known`-Abrufe, die ArgoCD nicht
kennt.
...
1. Eine aktuelle Sicherung nach `docs/backup-integration-guide.md` liegt vor —
   insbesondere der `dcs`-Datenbank, weil die gewrappten CEKs nur dort
   existieren.
...
Ein Rollback ist damit ein Werkzeug für Konfigurations- und Image-Rücknahmen,
nicht für Datenmodell-Rücknahmen. Diese Prozedur ist im Rahmen dieses
Dokuments nicht in einem laufenden Cluster erprobt worden; sie ist aus den
...
das schließt das Öffnen des PKCS#11-Tokens, das funktionale Katalog-Gate und
den Schema-Sync ein. Die Readiness-Probe fragt `/readyz` alle 2 Sekunden mit
1 Sekunde Timeout und 3 Fehlversuchen ab.

Ein Pod im Zustand `Running` mit `0/1 Ready` und einem Log, das
`bootstrap HTTP server listening ... (initializing)` zeigt, ist damit ein
**normaler** Zustand während der ersten Minuten nach einem Install — nicht
notwendigerweise ein Fehler.

**Liveness.** Der Backend-Container hat keine Liveness-Probe (A5.8). pdf-core
hat beides: Readiness und Liveness auf `/swagger.json`.

**Metriken.** Das Backend exponiert Prometheus-Metriken unter `/metrics` auf
der Wurzel des Service-Ports — außerhalb von `DCS_API_PATH`. Für das
Einsammeln ist ein eigenes ServiceMonitor-Objekt anzulegen; der im Chart
mitgelieferte ist auf die Testumgebung fixiert (A5.8).
...
| Render bricht ab mit `pdfCore.signing.signerKeyPEM is required ...` | `pkcs11.provisioning.enabled=false` und `pdfCore.signing.existingSecret` leer | `existingSecret` setzen — der geforderte private Schlüssel wird von nichts gelesen (A5.8) |
...
| Symptom | Ursache | Behebung |
| --- | --- | --- |
| Ein Job steht auf `Running` mit **null** Pods; Event: `forbidden: minimum cpu usage per Container is 100m, but request is 50m` | Eine `LimitRange` **weist ab**, sie klemmt nicht hoch. Der Job-Controller versucht es endlos weiter — das sieht aus wie ein Hänger, nicht wie eine Ablehnung | Die tatsächlichen Werte der LimitRange lesen (`kubectl -n <ns> describe limitrange`) und die Requests darüber setzen |
...
| `helm list` zeigt nichts, obwohl Objekte im Namespace liegen | Der Install wurde abgebrochen und steht auf `pending-install` — unsichtbar für `helm list`, sichtbar für `helm status` | `helm -n <ns> status <release>` prüfen; danach gezielt aufräumen oder mit `helm upgrade --install` adoptieren |
...
**Der Preis, und er ist nicht optional zu kennen:**
...
- **Es gibt keinen Helm-Release-Datensatz.** `helm list` zeigt nichts,
  ...
  zu adoptieren — die Besitzmarkierungen oben sind das, was das ermöglicht.
...
- das ORCE-Volume — und damit die **Archiv-Notar-Belegkette**
  (`/data/archive-audit-events.jsonl`, das, wogegen die
  Archiv-Integritätsprüfung vergleicht) sowie den Seriennummernzähler der
  lokalen TSA. Das Volume ist damit nicht bloße Bequemlichkeit; es trägt
  Nachweise, die sich aus dem Deployment nicht wiederherstellen lassen.
...
`oid4vp.trust.existingConfigMap` referenziert ist — sie wird nicht von Helm
verwaltet.
...
### A14.3 Zum mitgelieferten `uninstall.sh`

`deployment/helm/uninstall.sh` nimmt `<kubeconfig> <path>` und leitet daraus
Namespace `digital-contracting-service-<path>` und Release
`digital-contracting-service` ab. Beides ist fest verdrahtet und deckt sich
nicht mit den Vorgaben von `deploy.sh` (Release `dcs`, Namespace `dcs`). Das
Skript passt damit nur zu einem Deployment, das genau diesem Namensschema
folgt; zusätzlich löscht es den gesamten Namespace. Für alle anderen
Deployments ist die Prozedur aus A14.2 der richtige Weg.
...
- SoftHSM2 (`libsofthsm2.so`) — die Entwicklungsschlüssel liegen in einem
  PKCS#11-Token, nicht in Dateien
- `helm`, `kubectl`, `docker`
...
2. wartet auf den Federated Catalogue, falls die Werte-Datei ihn aktiviert —
   das Katalog-Readiness-Gate des Backends macht genau einen Versuch, ein zu
   früh gestarteter Backend-Prozess stirbt daran
...
6. provisioniert das lokale SoftHSM2-Token unter `$HOME/.dcs/softhsm-8991` und
   erzeugt `backend/certs/dev/did-8991.json` mit `gendid`
...
und pdf-core laufen nativ, nicht im Cluster. Deshalb wird auch kein
In-Cluster-Token provisioniert.
...
`/tmp/backend-live.log` enthält `HTTP server listening`. Beendet sich einer der
drei Prozesse, gibt `dev-stack.sh` die letzten 200 Logzeilen des betroffenen
...
1. Der Federated Catalogue muss antworten, bevor das Backend startet. Das
   Readiness-Gate macht genau **einen** Versuch — absichtlich, damit ein
   kaputter Katalog nicht durch Wiederholungen verdeckt wird.
2. **pdf-core muss laufen** (`http://localhost:8080`). Das Backend probiert
...
3. Der Status-List-Dienst muss erreichbar sein — ebenfalls mit einem
   Drei-Minuten-Fenster, danach fatal.

Der zweite Punkt ist der, der am häufigsten übersehen wird: pdf-core ist ein
eigener Prozess (`cd pdf-core && air`), kein Teil des Backends.
...
Makefile getrennt — zuerst auf das Secret, dann auf den Rollout.
...
Backend-Pod bleibt damit dauerhaft nicht bereit, obwohl der Backend-Container
...
`$HOME/.dcs-bdd-venv` an, nicht im Repository — ein venv auf dem 9P-Dateisystem
