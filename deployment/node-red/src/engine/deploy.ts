...
export function deploy(node: DeployNode, config: DeployConfig, msg: NodeMessage): void {
    ...
    const env = {
        ...
        DEP_HYDRA:               config.depHydra ? 'true' : 'false',
        DEP_HYDRA_DSN:           config.depHydraDsn || 'memory',
        DEP_HYDRA_SYSTEM_SECRET: config.depHydraSystemSecret || '',
        DEP_HYDRA_LOGIN_URL:     config.depHydraLoginUrl || '',
        DEP_HYDRA_CONSENT_URL:   config.depHydraConsentUrl || '',
